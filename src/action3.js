...
const jsonFilePath = path.join(__dirname, '../assets/moreHrContacts.json');
const countFilePath = path.join(__dirname, '../assets/hrContactsEmailCounter.txt');
const sentEmailsLogPath = path.join(__dirname, '../assets/hrContactsSentLog.json');
...
// Create email message - Updated to handle new contact format
function createEmailMessage(recipient, resumeData, jobRole = 'Web Developer Intern') {
    ...
    return {
        ...
        subject: `Application for ${jobRole} Internship at ${companyName}`,
        attachment: [
            {
                data: `<html>
                    <body>
                        <p>Dear ${recipientName},</p>
                        <p>I hope this email finds you well. My name is Hrishabh Joshi, and I am currently a BCA student at IPU. I am reaching out to express my interest in the <strong>${jobRole}</strong> internship at ${companyName}. I have attached my resume for your review.</p>
                        <p>As an aspiring Web Developer, I have gained hands-on experience with MERN stack along with ORMs and PostgreSQL through my academic projects and coursework. I am eager to apply my knowledge and further develop my skills in a real-world setting at ${companyName}.</p>
                        <p>I am excited about the opportunity to contribute to your team and learn from the experts at ${companyName}.</p>
    ...
    try {
        ...
        const emailPromises = batchRecipients.map(async (recipient) => {
            ...
            try {
                ...
                console.log(`✅ [${sentEmailCount}] Email sent to ${recipient.email} (${recipient.company || 'Unknown Company'})`);
            } catch (err) {
