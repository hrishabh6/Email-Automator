import { SMTPClient } from 'emailjs';
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import dotenv from 'dotenv';

// Configure environment variables
dotenv.config();

// Convert __dirname for ES Modules
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Configure SMTP client
const client = new SMTPClient({
    user: 'hrishabhjoshi123@gmail.com',
    password: process.env.EMAIL_PASSWORD,
    host: 'smtp.gmail.com',
    ssl: true,
});

// File paths
const jsonFilePath2 = path.join(__dirname, '../assets/company_data.json');
const countFilePath2 = path.join(__dirname, '../assets/sentEmailCountForHRs.txt');
const sentEmailsLogPath = path.join(__dirname, '../assets/sentEmailsLog.json');

// Constants
const MAX_EMAILS = 390;
const BATCH_SIZE = 10;

// Function to load sent emails log
function loadSentEmailsLog() {
    try {
        if (fs.existsSync(sentEmailsLogPath)) {
            return new Set(JSON.parse(fs.readFileSync(sentEmailsLogPath, 'utf8')));
        }
    } catch (error) {
        console.error('⚠️ Error reading sent emails log:', error);
    }
    return new Set();
}

// Function to save sent emails log
function saveSentEmailsLog(sentEmails) {
    try {
        fs.writeFileSync(sentEmailsLogPath, JSON.stringify([...sentEmails]), 'utf8');
    } catch (error) {
        console.error('⚠️ Error writing sent emails log:', error);
    }
}

// Function to get and update sent email count
function getSentEmailCount2() {
    try {
        if (fs.existsSync(countFilePath2)) {
            return parseInt(fs.readFileSync(countFilePath2, 'utf8'), 10) || 0;
        }
    } catch (error) {
        console.error('⚠️ Error reading sent email count:', error);
    }
    return 0;
}

function updateSentEmailCount2(count) {
    try {
        fs.writeFileSync(countFilePath2, count.toString(), 'utf8');
    } catch (error) {
        console.error('⚠️ Error writing sent email count:', error);
    }
}

// Create email message
function createEmailMessage(email, company, resumeData, jobRole = 'Web Developer Intern') {
    return {
        text: `Dear ${company},\n\nI hope this email finds you well...`, // Your existing email text
        from: 'Hrishabh Joshi <hrishabhjoshi123@gmail.com>',
        to: `${company} <${email}>`,
        subject: `Application for ${jobRole} Internship at ${company}`,
        attachment: [
            {
                data: `<html>
                    <body>
                        <p>Dear ${company},</p>
                        <p>I hope this email finds you well. My name is Hrishabh Joshi, and I am currently a BCA student at IPU. I am reaching out to express my interest in the <strong>${jobRole}</strong> internship at your esteemed company. I have attached my resume for your review.</p>
                        <p>As an aspiring Web Developer, I have gained hands-on experience with MERN stack along with ORMs and PostgreSQL through my academic projects and coursework. I am eager to apply my knowledge and further develop my skills in a real-world setting at ${company}.</p>
                        <p>I am excited about the opportunity to contribute to your team and learn from the experts at ${company}.</p>
                        <p>Best regards,<br>Hrishabh Joshi</p>
                        <p>Connect with me on:</p>
                        <ul>
                            <li><a href="https://www.hrishabhjoshi.xyz" target="_blank">Portfolio</a></li>
                            <li><a href="http://www.linkedin.com/in/hrishabh-joshi-61399a298" target="_blank">LinkedIn</a></li>
                            <li><a href="https://github.com/hrishabh6" target="_blank">GitHub</a></li>
                        </ul>
                    </body>
                </html>`,
                alternative: true,
            },
            {
                data: resumeData,
                type: 'application/pdf',
                name: 'Hrishabh_Joshi_Resume.pdf',
            },
        ],
    };
}

// Load sent email count and sent emails log
let sentEmailCount2 = getSentEmailCount2();
let sentEmails = loadSentEmailsLog();

async function sendJobApplications2() {
    try {
        if (sentEmailCount2 >= MAX_EMAILS) {
            console.log('✅ All emails have been sent. Stopping the script.');
            process.exit(0);
        }

        // Read JSON data and resume
        const recipients = JSON.parse(fs.readFileSync(jsonFilePath2));
        const resumeData = fs.readFileSync(path.join(__dirname, '../assets', 'resume.pdf'));

        // Filter out recipients whose emails have already been sent
        const remainingRecipients = recipients.filter(recipient => {
            const emails = [recipient.Email];
            if (recipient.Email2) emails.push(recipient.Email2);
            return emails.some(email => !sentEmails.has(email));
        });

        // Get recipients for this batch
        const batchRecipients = remainingRecipients.slice(0, BATCH_SIZE);

        if (batchRecipients.length === 0) {
            console.log('✅ No more recipients left. Stopping script.');
            process.exit(0);
        }

        // Process each recipient
        const emailPromises = batchRecipients.flatMap(recipient => {
            const emails = [recipient.Email];
            if (recipient.Email2) emails.push(recipient.Email2);

            return emails
                .filter(email => !sentEmails.has(email))
                .map(async (email) => {
                    if (sentEmailCount2 >= MAX_EMAILS) return;

                    const message = createEmailMessage(email, recipient.Company, resumeData);

                    try {
                        await client.sendAsync(message);
                        sentEmailCount2++;
                        sentEmails.add(email);
                        updateSentEmailCount2(sentEmailCount2);
                        saveSentEmailsLog(sentEmails);
                        console.log(`✅ [${sentEmailCount2}] Email sent to ${email} (${recipient.Company})`);
                    } catch (err) {
                        console.error(`❌ Failed to send email to ${email}:`, err);
                    }
                });
        });

        // Wait for all emails to be sent
        await Promise.all(emailPromises);

        if (sentEmailCount2 >= MAX_EMAILS) {
            console.log('✅ Maximum emails sent. Stopping the script.');
            process.exit(0);
        }
    } catch (error) {
        console.error('❌ Error processing emails:', error);
    }
}

// Run every 5 seconds
const interval = setInterval(() => {
    console.log('🚀 Running email batch...');
    sendJobApplications2();
}, 5000);